<x-slot name="title">{{ __('admin.doctors.title') }}</x-slot>

<div>
    <!-- Header -->
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('admin.doctors.title') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('admin.doctors.subtitle') }}</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.doctors.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                + {{ __('admin.doctors.create_doctor') }}
            </a>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <div class="flex items-center space-x-4">
            <input type="text" wire:model.live.debounce.300ms="search" 
                   placeholder="{{ __('admin.doctors.search_placeholder') }}"
            ...
            <label class="flex items-center space-x-2">
                ...
                <span class="text-sm text-gray-700">{{ __('admin.doctors.show_inactive') }}</span>
            </label>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.doctors.table.doctor') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.doctors.table.specialization') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.doctors.table.experience') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.doctors.table.fee') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.doctors.table.appointments') }}</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($doctors as $doctor)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $doctor->experience_years }} {{ __('admin.doctors.years') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                    ...
                                    {{ $doctor->is_available ? __('admin.doctors.available') : __('admin.doctors.unavailable') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <a href="{{ route('admin.doctors.schedule', $doctor->id) }}" class="text-green-600 hover:text-green-900">
                                        {{ __('admin.doctors.schedule') }}
                                    </a>
                                    <button wire:click="toggleAvailability({{ $doctor->id }})" class="text-yellow-600 hover:text-yellow-900">
                                        {{ $doctor->is_available ? __('admin.doctors.make_unavailable') : __('admin.doctors.make_available') }}
                                    </button>
                                    <button wire:click="deleteDoctor({{ $doctor->id }})" 
                                            wire:confirm="{{ __('admin.doctors.delete_confirm') }}"
